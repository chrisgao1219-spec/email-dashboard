...
const READ_ENDPOINTS = new Set(['config', 'stats', 'dashboard', 'subjects', 'action', 'weekly', 'compare', 'inspire', 'copy', 'templates', 'bundle', 'aiStatus', 'aiIdeas', 'strategy', 'calendar', 'digest', 'sheetUrl', 'registered', 'shopifyData', 'competitorEmail', 'diag', 'analysis', 'emailContent']);
...
export default async function handler(req, res) {
  ...
  if (endpoint && AI_ENDPOINTS.has(endpoint)) {
    ...
    try {
      const { handleAiIdeas, handleTemplate, handleInspire, handleScore, handleRewrite, handleDigest, handleCustomTemplate, handleGenerateCampaign, handleGenerateCalendar } = await import('./ai-handler.js');
...
请基于以上真实竞品数据给出建议，不要给出与这些竞品无关的跨行业建议。`;
      ...
      switch (endpoint) {
        ...
        case 'generateCalendar': {
          const data = await handleGenerateCalendar(params);
          return res.status(200).json(data);
        }
        default:
